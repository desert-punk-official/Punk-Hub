--========================================
-- Punk X Debugger (Fixed ZIndex & Layout)
...
SearchBox.Position = UDim2.new(0.02, 0, 0.13, 0) -- Moved up slightly
...
-- Scroll Frame (Expanded height slightly)
local ScrollFrame = Instance.new("ScrollingFrame")
ScrollFrame.Position = UDim2.new(0.02, 0, 0.19, 0)
ScrollFrame.Size = UDim2.new(0.96, 0, 0.58, 0) -- Increased height to push buttons down
...
local FilterRow = Instance.new("Frame", MainFrame); FilterRow.Size = UDim2.new(0.96, 0, 0.05, 0); FilterRow.Position = UDim2.new(0.02, 0, 0.78, 0); FilterRow.BackgroundTransparency = 1
local BtnFrame = Instance.new("Frame", MainFrame); BtnFrame.Size = UDim2.new(0.96, 0, 0.05, 0); BtnFrame.Position = UDim2.new(0.02, 0, 0.85, 0); BtnFrame.BackgroundTransparency = 1
local AdvRow = Instance.new("Frame", MainFrame); AdvRow.Size = UDim2.new(0.96, 0, 0.05, 0); AdvRow.Position = UDim2.new(0.02, 0, 0.92, 0); AdvRow.BackgroundTransparency = 1
...
local InfoBtn = mkBtn(FilterRow, "INFO", "accentInfo", 0, 0.15); local WarnBtn = mkBtn(FilterRow, "WARN", "accentWarn", 0.16, 0.15); local ErrorBtn = mkBtn(FilterRow, "ERROR", "accentError", 0.32, 0.15)
local TimestampBtn = mkBtn(FilterRow, "Time", "accentNeutral", 0.48, 0.12); local LineNumBtn = mkBtn(FilterRow, "Line", "accentNeutral", 0.61, 0.12); local RegexBtn = mkBtn(FilterRow, "Regex", "accentNeutral", 0.74, 0.12); local FontBtn = mkBtn(FilterRow, "A"..fontSize, "accentNeutral", 0.87, 0.12)

local Copy = mkBtn(BtnFrame, "Copy", "accentInfo", 0, 0.158); local Clear = mkBtn(BtnFrame, "Clear", "accentError", 0.168, 0.158); local Filter = mkBtn(BtnFrame, "Filter", "accentSuccess", 0.336, 0.158)
local AutoScroll = mkBtn(BtnFrame, "Scroll", "accentNeutral", 0.504, 0.158); local ExportBtn = mkBtn(BtnFrame, "Export", "accentInfo", 0.672, 0.158); local ThemeBtn = mkBtn(BtnFrame, "Theme", "accentNeutral", 0.84, 0.158)

PinBtn = mkBtn(AdvRow, "Pin", "accentWarn", 0, 0.245); ExcludeBtn = mkBtn(AdvRow, "Exclude", "accentError", 0.252, 0.245); local HistoryBtn = mkBtn(AdvRow, "History", "accentSuccess", 0.504, 0.245); local Close = mkBtn(AdvRow, "Close", "accentError", 0.756, 0.245)
...
ExcludeBtn.MouseButton1Click:Connect(function()
    ...
    if t == "" then
        ...
        for i, p in ipairs(excludePatterns) do
            ...
            local btn = Instance.new("TextButton", f); btn.Size = UDim2.new(0, 25, 0, 20); btn.Position = UDim2.new(1, -30, 0.5, -10); btn.BackgroundColor3 = btnColors.accentError; btn.Text = "X"; btn.TextColor3 = Color3.new(1, 1, 1); btn.ZIndex = 202; Instance.new("UICorner", btn)
